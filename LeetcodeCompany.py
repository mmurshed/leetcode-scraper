...
class LeetcodeCompany:
    ...
    def get_company_slugs(self):
        if self.config.overwrite or not os.path.exists(self.config.company_filepath):
            ...
            for company_tag in company_tags:
                company_slugs.add(company_tag['slug'])
            self.save_company_urls(company_slugs)
        else:
            company_slugs = self.load_company_slugs()
        return company_slugs
    
    def save_company_urls(self, company_slugs):
        with open(self.config.company_filepath, 'w') as file:
            for company_slug in company_slugs:
                file.write(f"{LeetcodeConstants.LEETCODE_URL}/company/{company_slug}/\n")
    
    def load_company_slugs(self):
        company_slugs = set()
        with open(self.config.company_filepath, 'r') as file:
            company_tags = file.readlines()
            for company_tag in company_tags:
                company_tag = company_tag.replace("\n", "").split("/")[-2]
                company_slugs.add(company_tag)
    ...
    def scrape_selected_company_questions(self, company_slug):        
        ...
        if not companys_slugs or company_slug not in companys_slugs:
            self.logger.error(f"Company not valid {company_slug}")
            return

        self.create_company_index_html(company_slug)
        self.scrape_company_question_data(company_slug)
    ...
    def scrape_all_company_questions(self):
        ...
        companys_slugs = self.get_company_slugs()
        self.create_all_company_index(companys_slugs)

        for company_slug in companys_slugs:
            self.create_company_index_html(company_slug)
            self.scrape_company_question_data(company_slug)
    
    def create_all_company_index(self, companys_slugs):
        self.logger.info("Creating company index.html")
        cols = 10
        rows = len(companys_slugs)//10 + 1
        html = ''
        company_idx = 0
        for _ in range(rows):
            html += '<tr>'
            for _ in range(cols):
                if company_idx < len(companys_slugs):
                    companys_slug = companys_slugs[company_idx]
                    html += f'''<td><a href="{companys_slug}/index.html">{companys_slug}</a></td>'''
                    company_idx += 1
            html += '</tr>'
    ...
    def create_company_index_html(self, company_slug):
        categoriesToSlug = self.lc.get_favorite_details_for_company(company_slug)
        if not categoriesToSlug:
            self.logger.error(f"Favorite details not found for {company_slug}")
            return

        favoriteSlugs = {item["favoriteSlug"]: item["displayName"] for item in categoriesToSlug['generatedFavoritesInfo']['categoriesToSlugs']}
        ...
        for favoriteSlug in favoriteSlugs:
            ...
            for question in company_questions:
                ...
                company_fav_dir  = os.path.join(self.config.companies_directory, company_slug, favoriteSlug)
                os.makedirs(company_fav_dir, exist_ok=True)

                frequency = round(float(question['frequency']), 1)            
                frequency_label = '{:.1f}'.format(frequency)
                question_title_format = LeetcodeUtility.qbasename(question_id, question['title'])
                question_fname = LeetcodeUtility.qhtml(question_id, question['title'])
                html += f'''<tr>
            ...
            with open(os.path.join(company_fav_dir, f"{favoriteSlug}.html"), 'w') as file:
                file.write(f"""<!DOCTYPE html>
                    <html lang="en">
                    <head> </head>
                    <body>
    ...
    def scrape_company_question_data(self, company_slug):
